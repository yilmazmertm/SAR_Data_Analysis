...
class Config:
    def __init__(self, dataset='sar_data'):
        if dataset == 'sar_data':
            # CNN (cnn)
...
def train(model, optimizer, train_loader, epoch):
    ...
    for batch_id, (data, target) in enumerate(tqdm(train_loader)):
        ...
        target = torch.sparse.torch.eye(5).index_select(dim=0, index=target)
...
if __name__ == '__main__':
    ...
    sar_data = Dataset(dataset, BATCH_SIZE)
    ...
    for e in range(1, N_EPOCHS + 1):
        train(capsule_net, optimizer, sar_data.train_loader, e)
        test(capsule_net, sar_data.test_loader, e)
